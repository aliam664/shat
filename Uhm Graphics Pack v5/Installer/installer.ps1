# =====================================================================
#  Uhm Graphics Pack v5 - Installer
...
try {
    ...
    $Host.UI.RawUI.WindowTitle = 'Uhm Graphics Pack v5 - Installer'
} catch {}
...
function Msg {
    param([string]$Text, [string]$Title = 'Uhm Graphics Pack v5', [string]$Icon = 'Information', [string]$Buttons = 'OK')
...
Write-Host '        Uhm Graphics Pack v5  -  نصب‌کننده هوشمند / نسخه ۵' -ForegroundColor Magenta
Write-Host '   ===============================================================' -ForegroundColor Magenta
Write-Host ''

Msg -Text ("به نصب‌کننده پک گرافیک Uhm نسخه ۵ خوش آمدید.`n`n" +
...
# v5: recommend the Day filter as the default active filter
$recommendedFilter = $recommendedFilter
foreach ($nm in $installedFilters) { if ($nm -match 'Day') { $recommendedFilter = $nm; break } }
    ...
    if ($m.Success) {
        ...
        if ($installedFilters -contains $curFilter) {
            ...
            $fix = Msg -Text ('فیلتر فعال فعلی بازی «' + $curFilter + '» است.' + "`n`n" +
                'برای بهترین نتیجه با پک، فیلتر «' + $recommendedFilter + '» پیشنهاد می‌شود.' + "`n" +
                'الان در تنظیمات ویدیوی بازی اعمال شود؟') -Buttons YesNo -Icon Question
            if ($fix -eq 'Yes') {
                $new = [regex]::Replace($vtxt, '(?mi)^FILTER=.*$', ('FILTER=' + $recommendedFilter))
                Copy-Item $videoIni ($videoIni + '.bak') -Force
                Set-Content -Path $videoIni -Value $new -Encoding UTF8 -NoNewline
                Ok ('فیلتر فعال ویدیو تغییر کرد به: ' + $recommendedFilter)
                Rep 'اصلاح: فیلتر فعال ویدیو -> ' + $recommendedFilter
            } else {
...
$dropDir = Join-Path $acRoot 'Uhm Pack v5 Presets'
...
$reportText = @(
    'Uhm Graphics Pack v5 - گزارش نصب',
    ...
    'انتخاب فیلتر (نسخه ۵ - سه فیلتر نصب شد):',
    '     - روز و هوای روشن => «Uhm Natural LCS v5 - Day» (پیشنهاد پیش‌فرض نصب‌کننده)',
    '     - شب و مسابقه زیر نور چراغ => «Uhm Night LCS v5 - Night»',
    '     - استایل رترو/سینمایی قدیمی => «NFS 2015 Edited By Uhm» (گزینه جایگزین)',
    '     تعویض فیلتر: در بازی، تنظیمات ویدیو > PP Filter یا کلید میانبر تعویض فیلتر.',
    '',
